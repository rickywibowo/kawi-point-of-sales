    ...
    actions: {
        async loadFromApi() {
            ...
            this.promotions = response.promotions?.map((promotion) => ({
                ...
                minimumSubtotal: Number(promotion.minimum_subtotal ?? 0),
                maximumDiscount: promotion.maximum_discount === null ? null : Number(promotion.maximum_discount ?? 0),
                startsOn: promotion.starts_on,
                endsOn: promotion.ends_on,
                isActive: Boolean(promotion.is_active ?? true),
            })) ?? this.promotions;
